...
The path to a module that runs some code to configure or set up the testing environment before each test. Option can be used mutliple times to run more than one file. Since every test runs in its own environment, these scripts will be executed in the testing environment immediately before executing the test code itself.
